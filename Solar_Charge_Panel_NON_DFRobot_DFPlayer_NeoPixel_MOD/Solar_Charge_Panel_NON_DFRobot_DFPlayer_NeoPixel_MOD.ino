...
#define THRESHOLD_0    12.48     // Sensor value below this threshold = Red
#define THRESHOLD_1    12.48     // Sensor value between this and Threshold 2 = Orange
#define THRESHOLD_2    13.44     // Sensor value above Threshold 1 = Orange
...
const long sensorInterval = 500; // .5 seconds in milliseconds
...
Serial.println("For Seth Hartmann");
...
delay(250);
   myDFPlayer.play(1); // Play sound 1
...
// Check if 30 seconds have passed since the last sensor check
  if (millis() - lastSensorCheck >= sensorInterval) {
    lastSensorCheck = millis(); // Update the last check time
    
    // Read the sensor value
    int sensorValue = analogRead(vIN1);
    Serial.print("NeoPixel Trigger: ");
    Serial.println(vIN1);
    
    // Decide what color to set the NeoPixel based on the sensor value
     for (int i = 0; i < NUM_PIXELS; i++) {
    if (vIN1 < THRESHOLD_0) {
      // Sensor value below Threshold 0, turn NeoPixel red
      strip.setPixelColor(i, strip.Color(255, 0, 0)); // Red
       Serial.print("NeoPixel Red");
    } 
    else if (vIN1 >= THRESHOLD_1 &&vIN1 <= THRESHOLD_2) {
      // Sensor value between Threshold 1 and Threshold 2, turn NeoPixel orange
     strip.setPixelColor(i, strip.Color(255, 215, 0)); // Orange
      Serial.print("NeoPixel Orange");
    } 
    else {
      // Sensor value outside of the defined thresholds, turn NeoPixel off
      strip.setPixelColor(i, strip.Color(0, 0, 0)); // Off
      Serial.print("NeoPixel Off");
    }
    
    // Update the NeoPixel strip to show the new color
    strip.show();
     }
  }
 ...
 */
    ...
    if (digitalRead(RECORD_BUTTON) == LOW && (currentMillis - lastPressTimeRecordbutton) >= DEBOUNCE_DELAY) {
        myDFPlayer.play(3); // Play sound 3
        Serial.println("Playing sound 3.");
        lastPressTimeRecordbutton = currentMillis;
        delay(500);
    }

    if (digitalRead(PLAY_BUTTON) == LOW && (currentMillis - lastPressTimePlaybutton) >= DEBOUNCE_DELAY) {
        myDFPlayer.play(2); // Play sound 2
        Serial.println("Playing sound 2.");
        lastPressTimePlaybutton = currentMillis;
        delay(500);
    }

    if (digitalRead(STOP_BUTTON) == LOW && (currentMillis - lastPressTimeStopbutton) >= DEBOUNCE_DELAY) {
        myDFPlayer.stop(); // Stop any sound playing
        Serial.println("Sound stopped.");
        lastPressTimeStopbutton = currentMillis;
        delay(500);
    }
